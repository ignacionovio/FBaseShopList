...
export default function HomeScreen(props) {
    ...
    const [ListarInfo, setListarInfo] = useState('Pendiente')
    ...
    const renderEntity = ({item, index}) => {
        ...
        const formatDate = () => {
            ...
            if (doneState) {
                if (item.doneAt == null) {
                    ...
                    var notFormattedDate = item.doneAt.toDate();
                ...
                if (item.createdAt == null) {
                    ...
                    var notFormattedDate = item.createdAt.toDate();
                    var formattedDate = notFormattedDate.toDateString();
                    var [, month, day, year] = formattedDate.split(' ');
                    var ddMmmYyyy = `${day}/${month}/${year}`;
                    fechaString = ddMmmYyyy.toString();
        ...
        const datosMuestro = () => {
            if ({doneState}) {
                return (formatDate(item.createdAt) - item.text)
            } else
            {
                return (formatDate(item.createdAt) - item.text)
            }
            ...
            <View style={styles.container}>
                ...
                {/* <View style={styles.footerView}>
                    ...
                    <TouchableOpacity style={styles.button} onPress={onFooterLinkPress}>
                        <Text style={styles.buttonText}>{ListarInfo}</Text>
                    </TouchableOpacity>
                { entities && (
